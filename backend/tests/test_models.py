...
# ---------------------------------------------------------------------------
# Phase 2 Block A — UserItem.excluded default
# ---------------------------------------------------------------------------


def test_user_item_excluded_has_python_default_false() -> None:
    """UserItem.excluded must default to False so ORM inserts don't require explicit value."""
    from sqlalchemy import inspect as sa_inspect

    from syncup.db.models import UserItem

    col = sa_inspect(UserItem).c.excluded
    assert col.default is not None, "UserItem.excluded must have a Python-side default"
    assert col.default.arg is False, "UserItem.excluded default must be False"
