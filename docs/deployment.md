...
```text
Caddy -> 127.0.0.1:8090 -> Next.js -> FastAPI -> PostgreSQL (`db`)
                              |
                              +-> /api/* and /uploads/* are internal rewrites
```

FastAPI (`3020`) and PostgreSQL are exposed only inside the Compose network.
The backend connects to PostgreSQL using the service name `db`; `localhost`
must not be used for this connection. PostgreSQL is never routed through Caddy.
The browser therefore sees one HTTPS origin for pages, API requests, uploads,
session cookies, and OAuth callbacks.
...
1. Copy `.env.example` to `.env` and set `POSTGRES_PASSWORD`, service
   credentials, and a production `SYNCUP_TOKEN_ENCRYPTION_KEY`. Adjust the
   public origin or host port if needed. This is the only environment file
   used by Compose.
